...
#include "iiwa_controller/SetTargetPosition.h"

using moveit::planning_interface::MoveItErrorCode;

bool tcp_move(iiwa_controller::SetTargetPosition::Request  &req,
              iiwa_controller::SetTargetPosition::Response &res) {
  ros::NodeHandle nh("~");
  ...
  while (ros::ok()) {
    if (my_iiwa.getRobotIsConnected()) {
      ...
      target_pose.position.y = req.target_position_meter.y;
...
int main (int argc, char **argv) {
  // Initialize ROS
  ros::init(argc, argv, "iiwa_tcp_move");
  ros::NodeHandle nh("~");
  
  ros::ServiceServer service = nh.advertiseService("tcp_move", tcp_move);
